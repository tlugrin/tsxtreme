      ...
      tol(0.001), v(-log(2*(1 - 0.99999999)))
{
    ...
    if(spec == tsxtreme::none){
        ...
        for(unsigned int reg = 0; reg < 8; reg++){
            curr.sd[0][reg] = (reg < 5)?sd_propa[reg]:sd_propb[reg - 5];
        }
    }
...
void ETfit::initialise(const double *start_a, const double *start_b){
    ...
    if(spec == tsxtreme::univariateMixture){
...
void ETfit::update_a(const unsigned int &iter){
    if(spec == tsxtreme::univariateMixture){
    ...
    }else if(spec == tsxtreme::firstOrderMarkov or spec == tsxtreme::none){
        unsigned int const d_max = (spec == tsxtreme::none)?nlag:1;
        ...
        for(unsigned int d = 0; d < d_max; d++){
            ...
            if(cond(a_star, curr.b[d], 0, d) and cond(a_star, curr.b[d], 1, d)){
                ...
                if(runif(0, 1) <= accept){
                    if(spec == tsxtreme::firstOrderMarkov){
                        for(unsigned int j = 0; j < nlag; j++){
                            curr.a[j] = pow(a_star,j+1.0);
                        }
                    }else{//if spec == tsxtreme::none
                        curr.a[d] = a_star;
                    }
...
double ETfit::loglik_diff(const double &star, const unsigned int &dim, const bool &alpha){
    ...
    unsigned int d_min = (spec == tsxtreme::none)?dim:0;//if Markov chain, then likelihood computed in one go
    unsigned int const d_max = (spec == tsxtreme::none)?(dim + 1):nlag;// otherwise: each dimension separately
    double a_star;
    ...
    for(unsigned int d = d_min; d < d_max; d++){
        for(unsigned int i = 0; i < n; i++){
            ...
            if(alpha){
                if(spec == tsxtreme::none){ a_star = star; }
...
void ETfit::run(const tsxtreme::algotype &type){
    rout("DEBUG: entering run()...\n");

    if(type == tsxtreme::conditional){
    ...
    if((it + 1)%2000 == 0){
        ...
        if(spec != tsxtreme::univariateMixture){
            ...
            rout("Adapted sd: %.2e | %.2e | %.2e | %.2e | %.2e || %.2e | %.2e | %.2e\n",
                 curr.sd[0][0], curr.sd[0][1], curr.sd[0][2], curr.sd[0][3],
                 curr.sd[0][4], curr.sd[0][5], curr.sd[0][6], curr.sd[0][7]);
        }
    }
    if(it + 1 == maxit)
